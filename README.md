# wh40k-list-builder (Kimi Code Skill)

战锤 40k(11th Edition)竞技军表组装助手 — 一个 Kimi Code CLI 的 Skill。
自动拉取最新单位分数、研究近期比赛冠军表、结合你自己的棋子库存,头脑风暴出 2~3 套构筑方向并最终生成一张可直接上桌的军表。

A Kimi Code CLI skill that builds competitive Warhammer 40,000 (11th Edition) army lists: it pulls the latest points data, researches recent tournament-winning lists, and brainstorms 2–3 archetypes based on your actual miniature collection before finalizing a table-ready list.

## 功能 Features

- **分数数据自动更新**:数据源为 [BSData/wh40k-11e](https://github.com/BSData/wh40k-11e),本地缓存 + 每次启动比对 GitHub commit 日期,有更新才重新下载(走 jsDelivr CDN,国内网络友好)。
...
把整个 `wh40k-list-builder` 目录复制到 Kimi Code 的用户技能目录:
...
# 或直接下载 ZIP 解压到 C:\Users\<你>\.kimi-code\skills\wh40k-list-builder
```

首次使用时会自动下载分数数据到 `data/` 目录(约 7 MB)。之后每次启动只检查更新,不重复下载。

## 使用 Usage

在 Kimi Code 里直接说,例如:

- `帮我组个 40k 的表`(触发 skill)
...
kimi-code, kimi-cli, skill, warhammer-40k, wh40k, 40k, 11th-edition, army-list, list-builder, dark-angels, space-marines, bsdata, munitorum-field-manual, balance-dataslate, tabletop, 战锤40k, 军表, 黑暗天使
